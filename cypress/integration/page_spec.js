        ...
        const testForWinning = (steps, times) => {
            ...
            if (steps === 0) {
                ...
                cy.get('#renderhere ol > li')
                    .contains('X-A-N-A')
                    .then(($obj) => {
                        if ($obj.get(0).innerText.indexOf('B-B-B-B') !== -1) {
                            console.log('inside winning condition', $obj)
      ...
      const playGame = (playWithSeq = ['Arsenic', 'Arsenic', 'Niagara', 'Niagara'], playSpace = mastermindInitialSequence, steps = 1) => {
          ...
          cy.get('#renderhere')
              .then(($obj) => {
                  ...
                  if (![iT.indexOf('B-B-B-B'), iT.indexOf('You win!')].includes(-1)) {
                      console.log([iT.indexOf('B-B-B-B'), iT.indexOf('You win!')]);
                      //I've won!
                      [0, 1, 2, 3].forEach((i) => {
                          cy.get('#renderhere select')
                              .eq(i)
                      ...
                      let containsStr = playWithSeq.map((s) => s.slice(0, 1)).join('-')
                      return cy
                          .get('#renderhere ol li:last-child:contains(' + containsStr + ')')
                          .then(($obj) => {
                              const inT = $obj.get(0).innerText;
                              const ix = inT.search(/[EWB]-[EWB]-[EWB]-[EWB]/);
                              const iT = inT.slice(ix, ix + 7).split('-');
